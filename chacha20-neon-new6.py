...
def testgen (i) :
    ...
    for k in range(2) :
        ...
        for j in range(1,4) :
            S = chr(65+j)
            if (k==0) : yield("    %s%d = vextq_u32(%s%d, %s%d, %d);" % (S,i,S,i,S,i,j))
            else :  yield("    %s%d = vextq_u32(%s%d, %s%d, %d);" % (S,i,S,i,S,i,4-j))
...
for j in range(0,R,2) : print(next(G[j]))
for i in range(1,LG) :
    for j in range(0,R,2) :
        print(next(G[j]))
        if (j+1<R) : print(next(G[j+1]))
for j in range(0,R,2) :
    if (j+1<R) : print(next(G[j+1]))
