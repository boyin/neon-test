...
int main(){
  ...
  crypto_stream_chacha20(out, OUTLEN, (void *)nonce, (void *)key);
  if (memcmp(cmp,out,OUTLEN)!=0) printf("Test failed!\n");
  ...
  for (i=0; i<REPS; i++) {
    ...
    c1 = hal_get_time();
    crypto_stream_chacha20(buf,BLOCKS*64,(void *)nonces,(void *)keys);
    c2 = hal_get_time();
    cc[i] = c2-c1;
    c += cc[i];
  }
