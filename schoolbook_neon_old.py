...
def schoolbook_neon_block (B) :
    ...
    for i in range(B) :
        ...
        for k in range(1,4) :
            for j in range(B) :
                print("      t%d = vmull_lane_s16(u%d,v%d,%d);" % (j,j,i,k))
            print("      t%d = vextq_s32(t%d, z, %d);" % (B,B-1,4-k))
            for j in range(B-1,0,-1) :
                print("      t%d = vextq_s32(t%d, t%d, %d);" % (j,j-1,j,4-k))
            print("      t0 = vextq_s32(z, t0, %d);" % (4-k))
            for j in range(B,-1,-1) :
                print("      a%d = vaddq_s32(a%d, t%d);" % (i+j,i+j,j))
            print("")
...
''' % (N, N2))
    ...
    print("  int32x4_t ", end="")
    for i in range(B) :
        print("a%d, a%d, t%d, " %(i*2,i*2+1,i), end="")
    print("t%d, z;" % (B))
